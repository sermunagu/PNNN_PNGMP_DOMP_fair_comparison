...
if ~isfield(pnnn, 'row') || ~istable(pnnn.row) || height(pnnn.row) ~= 1 || ...
        ~isequaln(linear.complexTable(complexIndex, :), selectedRows(1, :)) || ...
...
if hasFs && (~isnumeric(measurement.fs) || ~isscalar(measurement.fs) || ...
        ~isreal(measurement.fs) || ~isfinite(measurement.fs) || ...
        measurement.fs <= 0)
    error('run_selected_comparison:InvalidSampleRate', ...
        'measurement.fs must be a positive finite numeric scalar.');
end
if hasInfoFs && (~isnumeric(measurement.info_signal.fsovs) || ...
        ~isscalar(measurement.info_signal.fsovs) || ...
        ~isreal(measurement.info_signal.fsovs) || ...
        ~isfinite(measurement.info_signal.fsovs) || ...
        measurement.info_signal.fsovs <= 0)
    error('run_selected_comparison:InvalidSampleRate', ...
        'measurement.info_signal.fsovs must be a positive finite scalar.');
end
if hasFs
