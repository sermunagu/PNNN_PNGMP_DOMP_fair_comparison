function study = runPNNNComparisonStudy( ...
    x, y, split, cfg, targetActiveParams, studyMode)
% runPNNNComparisonStudy - Select, refit and evaluate the PNNN models.
% The optional sweep_n12 mode prepares only the shared N12 dense source;
% the default full mode preserves the historical three-model pipeline.

if nargin < 6
    studyMode = "full";
end
studyMode = string(validatestring(studyMode, {'full','sweep_n12'}));
includeH4 = studyMode == "full";
...
selection = selectHyperparameters(cfg, split, features, targets, rotation, ...
    y, h4, n12, targetActiveParams, seed, runtimeCfg, includeH4);
...
if includeH4
    ...
    h4Fit = refitFairPNNNDense(features, targets, rotation, y, ...
        identification, fullSignal, h4, seed, ...
        selection.h4.bestDenseEpoch, runtimeCfg.training);
end
...
if ~includeH4
    study = struct('selection', selection, 'budget', budget, ...
        'runtimeConfig', runtimeCfg, 'inputDimension', inputDimension, ...
        'features', features, 'targets', targets, 'rotation', rotation, ...
...
function selection = selectHyperparameters(cfg, split, features, targets, ...
    rotation, y, h4, n12, target, seed, runtimeCfg, includeH4)
