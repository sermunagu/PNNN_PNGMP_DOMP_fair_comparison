...
if isempty(targets) || any(targets <= 0) || any(mod(targets, 2)) || ...
    ...
    error('run_fixed_ridge_sweep:InvalidGrid', ...
        'Targets must be sorted unique positive even parameter counts.');
end
...
if any(~isfinite([identificationRows; fullSignalRows])) || ...
        any([identificationRows; fullSignalRows] ~= ...
        floor([identificationRows; fullSignalRows])) || ...
        any([identificationRows; fullSignalRows] < 1) || ...
        any([identificationRows; fullSignalRows] > numel(x))
    error('run_fixed_ridge_sweep:InvalidRows', ...
        'Split rows must be valid signal indices.');
end

requiredTableColumns = {'TargetRealParameters', ...
    'ActualRealParameters','FLOPsPerSample'};
if ~istable(linear.complexTable) || ~istable(linear.pnTable) || ...
        ~all(ismember(requiredTableColumns, ...
        linear.complexTable.Properties.VariableNames)) || ...
        ~all(ismember(requiredTableColumns, ...
        linear.pnTable.Properties.VariableNames)) || ...
        height(linear.complexTable) ~= numel(targets) || ...
        height(linear.pnTable) ~= numel(targets)
    error('run_fixed_ridge_sweep:InvalidPrincipalTables', ...
        'The two principal-family tables must contain every target once.');
end
...
if numel(complexPath) < maximumFeatures || numel(pnPath) < maximumFeatures || ...
...
for targetIndex = 1:numel(targets)
    ...
    if nnz(linear.complexTable.TargetRealParameters == targets(targetIndex)) ~= 1 || ...
            nnz(linear.pnTable.TargetRealParameters == targets(targetIndex)) ~= 1 || ...
            ~isequal(double(linear.supports.complex{targetIndex}(:)), ...
            expectedComplex) || ...
...
referenceTargetIndex = find(targets == 20, 1);
if isempty(referenceTargetIndex)
...
if targetEnergyIdentification <= 0 || targetEnergyFull <= 0
...
if any(featureNorms <= 0)
    error('run_fixed_ridge_sweep:ZeroPNFeature', ...
        'The saved PN-DOMP path contains a zero feature.');
end
