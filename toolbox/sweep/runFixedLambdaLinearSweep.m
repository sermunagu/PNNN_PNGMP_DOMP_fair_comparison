function fixed = runFixedLambdaLinearSweep( ...
    x, y, split, cfg, linear, storePredictions)
% runFixedLambdaLinearSweep - Refit stored linear sweep supports with fixed ridge.
...
if ~isscalar(storePredictions) || ~islogical(storePredictions)
    error('runFixedLambdaLinearSweep:InvalidStorageMode', ...
...
if numel(x) ~= numel(y) || isempty(x) || any(~isfinite(x)) || ...
    ...
    error('runFixedLambdaLinearSweep:InvalidInput', ...
...
if isempty(targets) || any(mod(targets, 2)) || ...
    ...
    error('runFixedLambdaLinearSweep:InvalidGrid', ...
...
if ~istable(linear.featureMetadata) || ...
    ...
    error('runFixedLambdaLinearSweep:InvalidMetadata', ...
...
if isempty(rows) || any(~isfinite(rows)) || any(rows ~= floor(rows)) || ...
    ...
    error('runFixedLambdaLinearSweep:InvalidRows', ...
...
if numel(complexPath) < max(counts) || numel(pnPath) < max(counts) || ...
    ...
    error('runFixedLambdaLinearSweep:IncompleteSupports', ...
...
for index = 1:numel(targets)
    ...
    if ~isequal(double(linear.supports.complex{index}(:)), ...
        ...
        error('runFixedLambdaLinearSweep:SupportMismatch', ...
...
if any(featureNorms <= 0)
    error('runFixedLambdaLinearSweep:ZeroPNFeature', ...
...
if any(~found)
    error('runFixedLambdaLinearSweep:PNFeatureMapping', ...
...
if targetEnergy <= 0
    error('runFixedLambdaLinearSweep:ZeroTargetEnergy', ...
...
for index = 1:numel(targets)
    ...
    if nnz(row) ~= 1
        error('runFixedLambdaLinearSweep:PrincipalRowMismatch', ...
