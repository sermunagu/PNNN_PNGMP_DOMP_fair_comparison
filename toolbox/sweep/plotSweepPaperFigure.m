...
if options.annotateSelected
    ...
    row = string(results.Model) == "Sparse PNNN N12" & ...
        results.ActualRealParameters == selectedParameters;
    if nnz(row) ~= 1
        error('plotSweepPaperFigure:MissingSelectedPoint', ...
            'The selected PNNN point must occur exactly once.');
    end
    selectedX = double(results.(xVariable)(row));
    selectedY = double(results.(options.metricVariable)(row));
    xline(axesHandle, selectedX, ':', 'Color', style.selectedRed, ...
        'LineWidth', 1.1, 'HandleVisibility', 'off');
    plot(axesHandle, selectedX, selectedY, 'o', ...
        'Color', style.selectedRed, 'MarkerFaceColor', style.selectedRed, ...
        'MarkerSize', style.markerSize + 2, 'LineWidth', 1.2, ...
        'DisplayName', 'Selected operating point');
    annotationText = sprintf( ...
        'Selected: %d params, %.0f FLOPs, %.2f dB NMSE', ...
        selectedParameters, options.selection.selectedPNNNFLOPs, ...
        options.selection.selectedPNNNNMSEdB);
    annotationHandle = text(axesHandle, selectedX, selectedY, ...
        annotationText, ...
        'Color', style.selectedRed, 'FontName', style.fontName, ...
        'FontSize', style.fontSize, 'HorizontalAlignment', 'right', ...
...
if options.isNMSE
    ...
    if options.annotateSelected
        annotationHandle.Position(2) = -30.5;
        annotationHandle.VerticalAlignment = 'top';
        annotationHandle.BackgroundColor = 'white';
        annotationHandle.Margin = 1;
    end
