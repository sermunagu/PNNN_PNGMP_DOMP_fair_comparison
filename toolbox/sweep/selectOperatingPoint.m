...
if ~isfield(selectionConfig, 'names')
    error('selectOperatingPoint:MissingNames', ...
        'The selection configuration must contain canonical public names.');
end
...
modelNames = [selectionConfig.names.complexGMPDOMP, ...
    selectionConfig.names.pniqGMP, selectionConfig.names.pnnn];
...
PNNNFutureGainDb = futureGainDb(:, 3);
...
summarySentence = compose([ ...
    ...
    'improvements are %.4f dB for %s, %.4f dB for %s, and %.4f dB ' ...
    'for %s. Because FLOPs increase ' ...
    ...
    windowParameters, futureGainDb(selectedRow, 1), modelNames(1), ...
    futureGainDb(selectedRow, 2), modelNames(2), ...
    futureGainDb(selectedRow, 3), modelNames(3));
...
selection = struct( ...
    ...
    'selectedPNNNFutureGainDb', futureGainDb(selectedRow, 3), ...
    ...
    'selectedPNNNFLOPs', flops(selectedRow, 3), ...
...
for modelIndex = 1:numel(modelNames)
    ...
    nmse(:, modelIndex) = double(family.FullSignalNMSEdB(order)); %#ok<AGROW>
    flops(:, modelIndex) = double(family.FLOPsPerSample(order)); %#ok<AGROW>
end
end
...
SelectedPNNNFutureGainDb = nan(rowCount, 1);
...
for windowIndex = 1:numel(windows)
    ...
    for toleranceIndex = 1:numel(tolerances)
        ...
        SelectedPNNNFutureGainDb(outputRow) = gainDb(selectedRow, 3);
...
sensitivity = table(StabilizationWindowParameters, ...
    ...
    SelectedPNIQFutureGainDb, SelectedPNNNFutureGainDb, ...
