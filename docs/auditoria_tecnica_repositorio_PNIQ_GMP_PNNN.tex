...
del pipeline Complex GMP--PN-IQ-GMP--Sparse PNNN}
...
\item rejilla \(\lambda=\{0,10^{-6},10^{-5},10^{-4},10^{-3},10^{-2}\}\);
...
train y el resto a validation usando una semilla fija.
...
\subsection{Ruta interna}

Se construyen \(U_{\mathrm{tr}}\) y \(U_{\mathrm{val}}\). DOMP obtiene una ruta
...
\[
D=\mathrm{diag}(\|u_1\|_2,\ldots,\|u_K\|_2),
\qquad
\widetilde U=UD^{-1}.
\]
...
\[
\widetilde h_0=\widetilde U^\dagger y.
\]
...
La lambda se elige por mínimo NMSE complejo en validation.

\subsection{Refit final}

La ruta se vuelve a seleccionar desde cero sobre toda identificación. Se
mantiene la lambda elegida. Se ajustan todos los prefijos y se evalúan en
identificación y señal completa.
...
La lambda es común a los dos ajustes y se selecciona minimizando el NMSE
complejo tras restaurar la fase.
...
El nombre recomendado es \textbf{PN-IQ-GMP}. DOMP es el método de selección,
no una variante ``PN-DOMP''.
...
Fuentes:
\code{toolbox/pnnn/buildPhaseNormInput.m} y
\code{buildPhaseNormDataset.m}.
...
Fuentes:
\code{fitFairPNNNDenseValidation.m},
\code{refitFairPNNNDense.m},
\code{scalePNNNTrainingBudget.m} y
\code{buildFairPNNNLayers.m}.
...
Fuentes:
\code{createMagnitudePruningMasks.m},
\code{fineTunePrunedNetwork.m},
\code{refitFairPNNNSparse.m} y
...
En identificación se conceptualiza:

\[
x_u=\frac{x}{\max_{n\in\mathcal I}|x[n]|},
\qquad
y_u=\frac{y}{\max_{n\in\mathcal I}|y[n]|}.
\]

Cada columna se normaliza:

\[
\bar u_k=\frac{u_k(x_u)}{\|u_k(x_u)\|_2}.
\]

Se ajustan los coeficientes en esa base. Para GMP:
...
La normalización de entrada es implícita en el código principal, porque el
factor global de una columna homogénea cancela al dividir por su norma. La
prueba lineal reconstruye la versión explícita y verifica igualdad.
...
\begin{tabular}{lrr}
\toprule
Modelo & NMSE full-signal & Mejora vs. GMP\\
\midrule
Complex GMP & -38.85998 & 0\\
WL-GMP & -38.86672 & 0.00674\\
PN-IQ & -42.74017 & 3.88019\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{lrrr}
\toprule
Familia & NMSE full-signal & FLOPs/muestra & Máx. escalar\\
\midrule
Complex GMP-DOMP & -38.85998 & 1807 & \(6.34896\times10^6\)\\
PN-IQ-GMP + DOMP & -42.74017 & 1095 & \(4.59256\times10^6\)\\
Sparse PNNN N12 & -39.24296 & 840 & 2.17243\\
\bottomrule
\end{tabular}
\end{center}
...
\item rejillas de parámetros y lambdas;
...
Una vez en train interno para la ruta usada al seleccionar lambda y otra vez
en toda identificación para el soporte final.
...
\subsection*{¿Los modelos sparse PNNN forman una trayectoria iterativa?}
...
Complex GMP & \code{toolbox/sweep/fit\_complex\_gmp\_domp.m}\\
PN-IQ & \code{toolbox/sweep/fit\_independent\_pniq\_domp.m}\\
Ridge & \code{toolbox/sweep/run\_fixed\_ridge\_sweep.m}\\
Features PNNN & \code{buildPhaseNormInput.m}, \code{buildPhaseNormDataset.m}\\
Arquitectura & \code{buildFairPNNNLayers.m}\\
Normalización & \code{computePNNNNormalization.m}\\
Dense validation & \code{fitFairPNNNDenseValidation.m}\\
Dense refit & \code{refitFairPNNNDense.m}\\
Máscaras & \code{createMagnitudePruningMasks.m}\\
Fine-tuning & \code{fineTunePrunedNetwork.m}\\
Sparse refit & \code{refitFairPNNNSparse.m}\\
...
\item[Sparse PNNN N12] Benchmark neural de 12 neuronas ocultas.
\item[Full-signal NMSE] NMSE sobre todo el registro, incluyendo identificación.
\item[Identification NMSE] NMSE sobre el bloque del 10\%.
\item[Internal validation] Subconjunto de identificación usado solo para selección.
\end{description}
