...
DOMP y PN-DOMP & \texttt{selectDOMPSupport.m}, \texttt{run\_linear\_sweep.m} & \texttt{linear\_sweep.mat} y soportes por presupuesto\\
Independent PN-IQ & \texttt{run\_linear\_sweep.m}, \texttt{fitIndependentIQGMP.m} & Curva PN-IQ y predicciones restauradas\\
Ridge fija & \texttt{run\_fixed\_ridge\_sweep.m} & \texttt{fixed\_lambda\_linear\_sweep.mat}\\
Sparse PNNN & \texttt{prepare\_pnnn\_dense\_source.m}, \texttt{fit\_sparse\_pnnn\_target.m} & Fuente N12 y puntos sparse ajustados con identificación completa\\
...
Reporte & \texttt{run\_selected\_comparison.m} & Tabla seleccionada y cuatro figuras espectrales\\
Tests & \texttt{run\_domp\_unit\_test.m}, \texttt{run\_scientific\_pipeline\_smoke\_test.m} & asserts PASS/FAIL\\
\bottomrule
\end{tabularx}
\end{table}
