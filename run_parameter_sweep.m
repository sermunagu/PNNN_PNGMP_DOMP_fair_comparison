...
if ~isfile(summaryFile)
    ...
    saved = load(summaryFile, 'checkpointArtifact');
    checkpointArtifact = saved.checkpointArtifact;
    if ~isequaln(checkpointArtifact.experimentSignature, ...
            cfg.experimentSignature) || ...
            ~isequaln(checkpointArtifact.sweepIdentity, sweepIdentity)
        error('run_parameter_sweep:ExperimentMismatch', ...
            'The summary checkpoint belongs to another experiment.');
    end
    summary = checkpointArtifact.payload;
    if ~isequal(sort(unique(string(summary.results.Model))), expectedModels)
        error('run_parameter_sweep:FamilyMismatch', ...
            'The summary checkpoint mixes model families.');
    end
end
result = summary;
