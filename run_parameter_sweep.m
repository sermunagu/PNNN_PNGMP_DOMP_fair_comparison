...
%% Run or reuse fixed-ridge references on the saved linear supports
fixedTimer = tic;
[fixedLinear, reusable] = updateSweepCheckpoint( ...
    resultDirectory, "fixed_linear", [], sweepIdentity, ...
    cfg.experimentSignature);
if ~reusable || ~validFixedLinearArtifact(fixedLinear, linear, cfg, split)
    fixedLinear = runFixedLambdaLinearSweep(x, y, split, cfg, linear);
...
results = writeSweepPresentationOutputs( ...
    linear, fixedLinear, pnnnRows, cfg.sweep.parameterGrid, resultDirectory);
...
function valid = validFixedLinearArtifact(value, linear, cfg, split)
...
expectedLambdas = [1e-3; 1e-4; 1e-5];
try
    rows = value.table;
    valid = isstruct(value) && all(isfield(value, required)) && ...
        istable(rows) && all(ismember(requiredColumns, ...
        rows.Properties.VariableNames)) && ...
        height(rows) == 6*numel(cfg.sweep.parameterGrid) && ...
        isequal(value.fixedLambdas(:), expectedLambdas) && ...
        isequaln(value.supports, linear.supports) && ...
        isequaln(value.paths, linear.paths) && ...
        value.reference.model == "Complex GMP-DOMP" && ...
        value.reference.targetRealParameters == cfg.sweep.parameterGrid(1) && ...
    ...
    models = unique(string(rows.Model));
    if valid
        valid = isequal(sort(models), ...
            sort(["Complex GMP-DOMP"; "PN-IQ PN-DOMP"]));
    end
    for model = models(:).'
        modelRows = string(rows.Model) == model;
        valid = valid && isequal(sort(unique(rows.FixedLambda(modelRows))), ...
            sort(expectedLambdas)) && ...
            isequal(sort(unique(rows.TargetRealParameters(modelRows))), ...
            sort(cfg.sweep.parameterGrid(:)));
    end
catch
    valid = false;
end
